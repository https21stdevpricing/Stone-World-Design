...
function buildReplyEmailHtml(data: AdminReplyData): string {
  return `
<!DOCTYPE html>
<html>
<head>
  <meta charset="utf-8" />
  <meta name="viewport" content="width=device-width, initial-scale=1.0" />
  <title>Message from Stone World</title>
</head>
<body style="margin:0;padding:0;background:#f5f5f5;font-family:'Helvetica Neue',Arial,sans-serif;">
  <table width="100%" cellpadding="0" cellspacing="0" style="background:#f5f5f5;padding:40px 0;">
    <tr>
      <td align="center">
        <table width="560" cellpadding="0" cellspacing="0" style="background:#ffffff;border-radius:8px;overflow:hidden;">
          ...
          <tr>
            <td style="padding:40px;">
              <p style="margin:0 0 8px;color:#999;font-size:13px;text-transform:uppercase;letter-spacing:1px;">Message from Our Team</p>
              <h1 style="margin:0 0 24px;font-size:24px;font-weight:600;color:#111;">Hi ${data.customerName},</h1>
              <p style="margin:0 0 4px;color:#555;font-size:14px;">Regarding your enquiry <strong style="color:#000;">${data.referenceNumber}</strong>:</p>
              <div style="background:#f8f8f8;border-left:4px solid #00B4B4;padding:20px 24px;border-radius:0 8px 8px 0;margin:20px 0 28px;color:#333;font-size:15px;line-height:1.7;">
                ${data.replyMessage.replace(/\n/g, "<br/>")}
              </div>
              <p style="margin:0 0 28px;color:#555;font-size:14px;line-height:1.6;">
                Feel free to reply to this email or call us directly for any further questions.
              </p>
              <p style="margin:32px 0 0;color:#aaa;font-size:12px;line-height:1.6;">
                You are receiving this because you submitted an enquiry with AB Stone World Pvt. Ltd.
              </p>
...
export async function sendAdminReply(data: AdminReplyData): Promise<void> {
  ...
  await transporter.sendMail({
    ...
    subject: `Re: Your enquiry ${data.referenceNumber} – Stone World`,
    html: buildReplyEmailHtml(data),
    text: `Hi ${data.customerName},\n\nRegarding your enquiry ${data.referenceNumber}:\n\n${data.replyMessage}\n\n– Stone World Team`,
  });
}
